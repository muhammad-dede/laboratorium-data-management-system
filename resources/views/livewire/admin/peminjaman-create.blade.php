<div>
    <form>
        ...
        <div class="form-group">
            <label for="id_siswa"><code>Siswa</code></label>
            <select wire:model="id_siswa" name="id_siswa" id="id_siswa"
                class="form-control rounded-0 @error('id_siswa') is-invalid @enderror" style="width: 100%;">
                <option value="" selected></option>
                @if ($data_siswa !== null)
                    @foreach ($data_siswa as $siswa)
                        <option value="{{ $siswa->id_siswa }}">{{ $siswa->nis }} - {{ $siswa->nama }}</option>
                    @endforeach
                @endif
            </select>
            @error('id_siswa')
